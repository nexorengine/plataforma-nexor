"""
NEXOR -- TRADUTOR 19 DOMINIOS CFE -- EN PARA ES v1
Traduz quiz_001_en.json para quiz_001_es.json
nos 19 dominios CFE recem gerados.
Blocos de 10 questoes por chamada -- sem truncamento.

USO:
    python traduzir_19_dominios_es.py
"""
...
def translate_block(client, questions):
    prompt = f"""Translate these CFE exam questions from English to Espanol neutro latinoamericano.
...
def translate_domain(client, domain_id):
    ...
    es_path = Path(QUIZZES_DIR) / domain_id / "quiz_001_es.json"
    ...
    for i, start in enumerate(blocks):
        ...
        try:
            result = translate_block(client, bloco)
    ...
    quiz_es = dict(data_en)
    quiz_es["lang"] = "es"
    quiz_es["questions"] = translated
    save_json(str(es_path), quiz_es)
    print(f"  ES: {len(translated)}q salvo ✅")
    return True

def main():
    print("=" * 70)
    print("  NEXOR -- TRADUTOR 19 DOMINIOS CFE -- EN PARA ES v1")
