...
def generate_mini_app(domain, q1en, q1pt, q1es, fc_merged, badge_b64):
    ...
    # ensure_ascii=False — chars Unicode diretos no JSON
    # HTML salvo como UTF-8 com meta charset UTF-8
    def to_json(obj):
        return json.dumps(obj, ensure_ascii=False, separators=(',',':'))

    js_q1en = to_json(q1en)
    js_q1pt = to_json(q1pt)
    js_q1es = to_json(q1es)
    js_fc   = to_json(fc_merged)
...
<script>
var QS1 = {{
  en: JSON.parse(document.getElementById('d-q1en').textContent),
  pt: JSON.parse(document.getElementById('d-q1pt').textContent),
  es: JSON.parse(document.getElementById('d-q1es').textContent)
}};
var QS2 = {{en:[],pt:[],es:[]}};
var FC  = JSON.parse(document.getElementById('d-fc').textContent);
...
window.addEventListener('load',function(){{
  ...
  buildQ();applyLang();renderFC();updateHero();
    ...
    # Injeta dados via split — evita qualquer conflito com f-string
    parts = html.split('__DATA_SCRIPTS__')
    if len(parts) == 2:
        data_scripts = (
            '<script type="application/json" id="d-q1en">' + js_q1en + '</script>\n' +
            '<script type="application/json" id="d-q1pt">' + js_q1pt + '</script>\n' +
            '<script type="application/json" id="d-q1es">' + js_q1es + '</script>\n' +
            '<script type="application/json" id="d-fc">'   + js_fc   + '</script>'
        )
        html = parts[0] + data_scripts + parts[1]
    else:
        print(f"  AVISO: __DATA_SCRIPTS__ nao encontrado no template ({len(parts)} partes)")
    return html
...
def main():
    ...
    for i, domain in enumerate(DOMAINS):
        ...
        try:
            q1en = json.load(open(str(q1_en_path), encoding="utf-8-sig"))["questions"]
            q1pt = json.load(open(str(q1_pt_path), encoding="utf-8-sig"))["questions"]
            q1es = json.load(open(str(q1_es_path), encoding="utf-8-sig"))["questions"]
