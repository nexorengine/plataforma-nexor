"""
gerar_quiz002_lote1.py — NEXOR FractalQuiz · Lote 1 (13 domínios)
Gera quiz_002 PT/EN/ES para os primeiros 13 domínios CFE pendentes.

Uso:
    python gerar_quiz002_lote1.py
    python gerar_quiz002_lote1.py --dry-run
    python gerar_quiz002_lote1.py --domain government_fraud
"""
...
LOG_FILE = Path(r"C:\ARAGORN\aragorn_quiz\quiz002_lote1_log.txt")
...
DOMAINS = [
    {"key": "government_fraud",           "name": "Government & Public Sector Fraud"},
    {"key": "identity_theft_cyberfraud",  "name": "Identity Theft & Cyberfraud"},
    {"key": "healthcare_fraud",           "name": "Health Care Fraud"},
    {"key": "payment_fraud",              "name": "Payment Fraud"},
    {"key": "securities_fraud",           "name": "Securities & Investment Fraud"},
    {"key": "report_writing",             "name": "Report Writing for Fraud Examiners"},
    {"key": "procurement_contract_fraud", "name": "Procurement & Contract Fraud"},
    {"key": "non_criminal_actions",       "name": "Non-Criminal Actions in Fraud Cases"},
    {"key": "investigation_planning",     "name": "Planning & Conducting Fraud Examination"},
    {"key": "legal_system_overview",      "name": "Overview of the Legal System"},
    {"key": "legal_issues_investigations","name": "Legal Issues in Investigations"},
    {"key": "insurance_fraud",            "name": "Insurance Fraud"},
    {"key": "individual_rights",          "name": "Individual Rights During Examinations"},
]
...
def main():
    ...
    targets = [d for d in DOMAINS if d["key"] == args.domain] if args.domain else DOMAINS

    log("=" * 64)
    log(f"NEXOR · FractalQuiz · Lote 1 · {len(targets)} domínios · {'DRY-RUN' if args.dry_run else 'EXECUTANDO'}")
    log("=" * 64)

    ok = err = 0
    for i, domain in enumerate(targets, 1):
        log(f"\n[{i:02d}/{len(targets)}] {domain['key']}")
        for lang in LANGS:
            try:
