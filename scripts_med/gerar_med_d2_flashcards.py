...
OUTPUT_PATH  = r"C:\ARAGORN\aragorn_quiz\static\flashcards\med\cirurgia_geral\flashcards_d2_pt.json"
LOG_PATH     = r"C:\ARAGORN\aragorn_quiz\scripts_med\log_d2_flashcards.txt"
...
TOPICOS = [
    ...
    "Pancreatite aguda — classificação e conduta",
    "Pancreatite crônica",
...
def gerar_camada(client, camada_codigo, camada_descricao, topicos):
    prompt = f"""Gere exatamente 12 flashcards de nível {camada_codigo} ({camada_descricao}) sobre Doenças Hepatobiliares e Pâncreas em Cirurgia Geral para prova de residência médica.
...
def main():
    ...
    for card in todos_cards:
        if len(card["frente"].split()) > 15:
            log(f"  ALERTA frente longa: {card['id']}")
            alertas += 1
        if len(card["verso"].split()) > 25:
            log(f"  ALERTA verso longo: {card['id']}")
            alertas += 1
    if alertas == 0:
