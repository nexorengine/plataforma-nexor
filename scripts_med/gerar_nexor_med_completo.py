"""
gerar_nexor_med_completo.py
Gera as 3 camadas completas do NEXOR MED — Cirurgia Geral
  Camada 1: Portal NEXOR MED (index_med.html)
  Camada 2: Dashboard Cirurgia Geral (cirurgia_geral/index.html)
  Camada 3: 9 mini-apps por domínio
"""
import json, os
...
DOMINIOS = [
    {"id":"d1","key":"abdome_agudo",        "name":"Abdome Agudo",                    "code":"D01","emoji":"🔴","color":"#8b1a1a",
     "flash":"flashcards_pt.json",   "q1":"quiz_001_pt.json",     "q2":"quiz_002_pt.json"},
    {"id":"d2","key":"hepatobiliar",         "name":"Hepatobiliar e Pâncreas",         "code":"D02","emoji":"🟡","color":"#7a6010",
     "flash":"flashcards_d2_pt.json","q1":"quiz_d2_001_pt.json",  "q2":"quiz_d2_002_pt.json"},
    {"id":"d3","key":"trauma",               "name":"Trauma",                          "code":"D03","emoji":"🟠","color":"#7a3a10",
     "flash":"flashcards_d3_pt.json","q1":"quiz_d3_001_pt.json",  "q2":"quiz_d3_002_pt.json"},
    {"id":"d4","key":"perioperatorio",       "name":"Perioperatório",                  "code":"D04","emoji":"🔵","color":"#10407a",
     "flash":"flashcards_d4_pt.json","q1":"quiz_d4_001_pt.json",  "q2":"quiz_d4_002_pt.json"},
    {"id":"d5","key":"hernias",              "name":"Parede Abdominal e Hérnias",      "code":"D05","emoji":"⚪","color":"#3a4a5a",
     "flash":"flashcards_d5_pt.json","q1":"quiz_d5_001_pt.json",  "q2":"quiz_d5_002_pt.json"},
    {"id":"d6","key":"tgi_superior",         "name":"Trato Digestivo Superior",        "code":"D06","emoji":"🟢","color":"#1a5a2a",
     "flash":"flashcards_d6_pt.json","q1":"quiz_d6_001_pt.json",  "q2":"quiz_d6_002_pt.json"},
    {"id":"d7","key":"tgi_inferior",         "name":"TGI Inferior e Coloproctologia",  "code":"D07","emoji":"🟤","color":"#5a3a1a",
     "flash":"flashcards_d7_pt.json","q1":"quiz_d7_001_pt.json",  "q2":"quiz_d7_002_pt.json"},
    {"id":"d8","key":"vascular",             "name":"Cirurgia Vascular",               "code":"D08","emoji":"🔷","color":"#1a3a6a",
     "flash":"flashcards_d8_pt.json","q1":"quiz_d8_001_pt.json",  "q2":"quiz_d8_002_pt.json"},
    {"id":"d9","key":"queimaduras",          "name":"Queimaduras",                     "code":"D09","emoji":"🔥","color":"#7a2a1a",
...
:root{
  ...
  --med:#8b1a1a;--med2:#a52020;--med3:#c0392b;
}
...
.hdr-badge{background:var(--bg2);border:1px solid var(--border2);border-radius:4px;padding:3px 8px;font-size:9px;letter-spacing:2px;color:var(--med2);text-transform:uppercase;}
...
.slabel{font-size:9px;letter-spacing:2.5px;color:var(--text3);text-transform:uppercase;margin-bottom:10px;}
"""
...
# ═══════════════════════════════════════════════════════════════════════════
# CAMADA 1 — PORTAL NEXOR MED
# ═══════════════════════════════════════════════════════════════════════════
def gerar_portal():
    areas = [
        {"name":"CIRURGIA GERAL",        "sub":"9 domínios · Residência Médica","color":"#8b1a1a","border":"#a52020","link":"med/cirurgia_geral/index.html","status":"active"},
        {"name":"GINECO & OBSTETRÍCIA",   "sub":"Em breve","color":"#3a1a5a","border":"#5a2a8a","link":"#","status":"soon"},
        {"name":"PEDIATRIA",             "sub":"Em breve","color":"#5a1a1a","border":"#8a2a2a","link":"#","status":"soon"},
        {"name":"CLÍNICA MÉDICA",        "sub":"Em breve","color":"#5a4a10","border":"#8a6a20","link":"#","status":"soon"},
        {"name":"MEDICINA PREVENTIVA",   "sub":"Em breve","color":"#2a2a2a","border":"#4a4a4a","link":"#","status":"soon"},
    ]
    ...
    for a in areas:
        if a["status"] == "active":
            cards_html += f"""
    <a href="{a['link']}" class="area-card active" style="border-color:{a['border']};background:linear-gradient(135deg,#0d1120,{a['color']}22);">
      <div class="area-top" style="background:{a['color']};"></div>
      <div class="area-name">{a['name']}</div>
      <div class="area-sub">{a['sub']}</div>
      <div class="area-badge" style="border-color:{a['border']};color:{a['border']};">DISPONÍVEL</div>
    </a>"""
        else:
            cards_html += f"""
    <div class="area-card soon">
      <div class="area-top" style="background:{a['color']};opacity:.4;"></div>
      <div class="area-name" style="opacity:.5;">{a['name']}</div>
      <div class="area-sub">{a['sub']}</div>
      <div class="area-badge soon-badge">EM BREVE</div>
    </div>"""
...
<title>NEXOR MED — Residência Médica</title>
{FONTS}
<style>
{CSS_BASE}
.hero{{text-align:center;padding:2.5rem 0 2rem;}}
.hero-label{{font-size:9px;letter-spacing:3px;color:var(--med2);text-transform:uppercase;margin-bottom:12px;}}
.hero-title{{font-size:32px;font-weight:700;color:#e8f0fa;letter-spacing:1px;margin-bottom:6px;}}
.hero-title span{{color:var(--med2);}}
.hero-sub{{font-size:12px;color:var(--text2);letter-spacing:1px;margin-bottom:2rem;}}
...
.area-grid{{display:grid;grid-template-columns:repeat(auto-fill,minmax(240px,1fr));gap:14px;margin:2rem 0;}}
.area-card{{background:var(--bg2);border:1px solid var(--border);border-radius:12px;padding:1.5rem;position:relative;overflow:hidden;text-decoration:none;color:inherit;display:block;transition:all .2s;}}
.area-card.active{{cursor:pointer;}}
.area-card.active:hover{{transform:translateY(-2px);box-shadow:0 8px 24px rgba(0,0,0,.4);}}
.area-card.soon{{opacity:.6;cursor:default;}}
.area-top{{height:3px;position:absolute;top:0;left:0;right:0;}}
.area-name{{font-size:15px;font-weight:700;color:#e8f0fa;margin:8px 0 6px;letter-spacing:.5px;}}
.area-sub{{font-size:10px;color:var(--text3);margin-bottom:12px;}}
.area-badge{{display:inline-block;border:1px solid;border-radius:3px;padding:2px 8px;font-size:9px;letter-spacing:1.5px;font-weight:600;}}
.soon-badge{{border-color:var(--border2);color:var(--text3);}}
...
{cards_html}
  ...
  <div class="footer">
    <div class="footer-text">NEXOR MED · nexorengine.com · FractalLearning™</div>
  </div>
</div>
</body>
</html>"""
...
# ═══════════════════════════════════════════════════════════════════════════
# CAMADA 2 — DASHBOARD CIRURGIA GERAL
# ═══════════════════════════════════════════════════════════════════════════
def gerar_dashboard():
    rows = ""
    for d in DOMINIOS:
    ...
    <a href="../../mini_apps/med/cirurgia_geral/{d['key']}/index.html" class="domain-row">
      <div class="domain-row-left">
        <div class="domain-row-emoji">{d['emoji']}</div>
        <div>
  ...
  try{{
    ...
    if(el){{
      if(q1&&q2){{const d1=JSON.parse(q1),d2=JSON.parse(q2);const avg=Math.round((d1.correct/d1.total+d2.correct/d2.total)/2*100);el.textContent=avg+'%';el.style.color=avg>=80?'#27ae60':avg>=60?'#d4a017':'#c0392b';}}
      else if(q1){{const d1=JSON.parse(q1);const p=Math.round(d1.correct/d1.total*100);el.textContent=p+'%';el.style.color=p>=80?'#27ae60':p>=60?'#d4a017':'#c0392b';}}
    }}
  }}catch(e){{}}"""
...
<style>
{CSS_BASE}
.cert-header{{display:flex;align-items:center;gap:14px;margin-bottom:1.5rem;padding-bottom:1rem;border-bottom:1px solid var(--border);}}
...
.domain-row{{background:var(--bg2);border:1px solid var(--border);border-radius:10px;padding:1.1rem 1.3rem;cursor:pointer;transition:all .2s;display:flex;align-items:center;justify-content:space-between;text-decoration:none;color:inherit;margin-bottom:10px;}}
.domain-row:hover{{border-color:var(--med2);background:var(--bg3);}}
.domain-row-left{{display:flex;align-items:center;gap:14px;}}
.domain-row-emoji{{font-size:22px;}}
...
<style>
{CSS_BASE}
:root{{--accent:{color};--accent2:{color}cc;}}
.screen{{display:none;}}.screen.active{{display:block;}}
.domain-title{{font-size:20px;font-weight:700;color:#e8f0fa;margin-bottom:4px;}}
.domain-sub{{font-size:10px;color:var(--text3);letter-spacing:1px;margin-bottom:1.5rem;}}
...
.quiz-sel-grid{{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:1.5rem;}}
...
<div class="hdr">
  ...
  <a href="../../index.html" class="hdr-back">← Domínios</a>
</div>

<!-- MENU -->
<div id="s-menu" class="screen active">
  <div class="domain-title">{emoji} {name}</div>
  <div class="domain-sub">CIRURGIA GERAL · {code} · NEXOR MED · PT</div>
  <div class="menu-grid">
    <div class="menu-card" onclick="startFlashcards()">
      <div class="menu-card-icon">🃏</div>
      <div class="menu-card-title">FractalFlashcards</div>
      <div class="menu-card-meta">48 cards · 4 camadas</div>
      <div class="menu-card-badge">F1 · F2 · F3 · F4</div>
    </div>
    <div class="menu-card" onclick="showQuizSel()">
      <div class="menu-card-icon">📋</div>
...
<div id="s-qsel" class="screen">
  <div class="domain-title">Selecionar Quiz</div>
  <div class="domain-sub">{name}</div>
  <div class="quiz-sel-grid">
    <div class="qsel-card" onclick="startQuiz(0)">
      <div class="saved-tag" id="sv1">SALVO</div>
      <div class="qsel-num">①</div>
...
<div id="s-quiz" class="screen">
  ...
  <div class="quiz-feedback" id="qf">
    <div class="fb-correct" id="fbc"></div>
    <div class="fb-wrong" id="fbw"></div>
  </div>
...
function startQuiz(idx){{curQ=idx;qdata=QZ[idx];qi=0;qok=0;qans=new Array(qdata.length).fill(null);show('s-quiz');renderQ();}}
function renderQ(){{
  ...
  (q.options||[]).forEach((o,i)=>{{const b=document.createElement('button');b.className='quiz-opt';b.textContent=o;b.onclick=()=>ans(i+1,b,q);oo.appendChild(b);}});
...
function ans(ch,btn,q){{
  ...
  document.getElementById('fbw').textContent=q.justification_wrong||'';
...
function showSC(){{
  ...
  let lc,lt;
  if(pct>=80){{lc='ready';lt='★★★ EXAM READY';}}else if(pct>=60){{lc='progress';lt='★★☆ IN PROGRESS';}}else{{lc='training';lt='★☆☆ TRAINING';}}
  let msg='';
  ...
  else msg='<strong>Continue estudando {name}.</strong> '+(ep<70?'Revise os conceitos básicos antes de avançar.':'Você domina o básico — aprofunde nas Standard e Hard.');
...
def main():
    print("=" * 60)
    print("NEXOR MED — Gerador Completo 3 Camadas")
    print("=" * 60)
    ...
    print(f"""
{'='*60}
CONCLUÍDO — 3 camadas geradas:

  C1: static/index_med.html
  C2: static/med/cirurgia_geral/index.html
  C3: static/mini_apps/med/cirurgia_geral/{{dominio}}/index.html

PRÓXIMOS PASSOS:
  1. Copiar badges para static/badges/med/
  2. Abrir index_med.html no browser para testar
  3. git add . && git commit -m "feat: NEXOR MED Cirurgia Geral MVP"
  4. git push
{'='*60}""")
