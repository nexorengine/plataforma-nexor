...
SUBDOMINIO   = "trauma"
DOMAIN_NAME  = "D3 · Trauma"
CERT_NAME    = "Residência Médica — Cirurgia Geral"
QUIZ_NUM     = 1
LANG         = "pt"
OUTPUT_PATH  = r"C:\ARAGORN\aragorn_quiz\static\quizzes\med\cirurgia_geral\quiz_d3_001_pt.json"
LOG_PATH     = r"C:\ARAGORN\aragorn_quiz\scripts_med\log_d3_quiz001.txt"
...
TOPICOS = [
    "Atendimento inicial ao politraumatizado — ATLS (ABCDE)",
    "Trauma abdominal — contuso e penetrante",
    "Trauma torácico — pneumotórax, hemotórax, tamponamento cardíaco",
    "Trauma cranioencefálico — TCE leve, moderado e grave",
    "Trauma vascular e controle de hemorragia",
    "Tratamento não operatório (TNO) no trauma",
    "Damage control surgery",
    "Trauma urológico e pélvico"
]
...
def shuffle_options(questoes):
    """Embaralha opções e recalcula correct para distribuição uniforme."""
    letras = ["A", "B", "C", "D"]
    for q in questoes:
        ...
        # Extrai textos sem prefixo
        textos = []
        for opt in options:
            if len(opt) > 2 and opt[1] == ".":
                textos.append(opt[2:].strip())
            else:
                textos.append(opt)
        random.shuffle(textos)
        new_correct = textos.index(correct_text[2:].strip() if len(correct_text) > 2 and correct_text[1] == "." else correct_text)
        q["options"] = [f"{letras[i]}. {textos[i]}" for i in range(len(textos))]
        q["correct"] = new_correct + 1
    return questoes
...
Referência: Sabiston 2019 + ATLS 10ª edição. Nível: ACM/SC.
{METODO_NEXOR}
Responda SOMENTE com JSON válido, sem markdown, sem backticks."""

    prompt = f"""Gere exatamente {n} questões nível {difficulty} ({camada} — {descricao}) sobre Trauma em Cirurgia Geral.
...
def main():
    ...
    # SHUFFLE embutido
    log("Aplicando shuffle nas respostas...")
    random.seed(None)  # seed aleatório
