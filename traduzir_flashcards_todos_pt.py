"""
NEXOR -- TRADUTOR FLASHCARDS CFE -- EN PARA PT v1
Traduz flashcards_en.json para flashcards_pt.json
para todos os 46 dominios CFE.
Pula dominios que ja tem PT.
Blocos de 10 cards por chamada.

USO:
    python traduzir_flashcards_todos_pt.py
"""
...
def translate_block(client, cards):
    prompt = f"""Translate these CFE flashcards from English to Portugues (Brasil).
...
[{{
  "id": 1,
  "topic": "same_as_input",
  "front": {{"pt": "Translated front text"}},
  "back": {{"pt": "Translated back text"}}
}}]"""
...
def translate_domain(client, domain_id):
    ...
    pt_path = Path(FLASHCARD_DIR) / domain_id / "flashcards_pt.json"

    if not en_path.exists():
        print(f"  FALTA EN: {domain_id}")
        return False

    # Pula se ja tem PT completo
    if pt_path.exists():
        data = load_json(str(pt_path))
        if len(data.get("cards", [])) >= 45:
            print(f"  SKIP {domain_id} (ja tem PT)")
            return True
    ...
    print(f"  PT: {len(translated)} cards salvos ✅")
    return True

def main():
    print("=" * 70)
    print("  NEXOR -- TRADUTOR FLASHCARDS CFE -- EN PARA PT v1")
    ...
    print(f"  Proximo: python traduzir_flashcards_todos_es.py")
    print("=" * 70)
